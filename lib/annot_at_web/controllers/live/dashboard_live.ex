  ...
  def render(assigns) do
    ...
    hover:-translate-y-0.5 active:translate-y-0 sm:self-auto"
      ...
      <nav class="mt-6 flex flex-wrap items-center gap-2">
        <.tab_link patch={~p"/dashboard?tab=setup"} active={@tab == :setup}>
          Set up ({length(@sites)})
        </.tab_link>
        <.tab_link patch={~p"/dashboard?tab=discovered"} active={@tab == :discovered}>
       ...
       border-ink/30 p-4 opacity-60"
            >
              <div class="min-w-0 flex-1">
                ...
                <div class="mt-0.5 text-sm text-ink/45">Publication missing from your repo</div>
              </div>
